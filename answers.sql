...
SELECT customers.email FROM customers JOIN orders ON customers.id = orders.customer_id  GROUP BY customers.email, orders.customer_id, orders.num_cupcakes HAVING SUM(num_cupcakes) >= 100  ORDER BY REVERSE(email); 
...
SELECT customers.fname, customers.lname, customers.email FROM customers  JOIN  orders ON customers.id = orders.customer_id JOIN cupcakes ON orders.cupcake_id = cupcakes.id WHERE orders.cupcake_id ='5' AND orders.processed ='True' GROUP BY customers.fname, customers.lname, customers.email ORDER BY customers.email;